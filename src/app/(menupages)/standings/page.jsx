...
const F1ChampionshipStandings = () => {
  ...
  const fetchStandings = async () => {
    try {
      ...
      setError(
        "Failed to load championship standings. Please try again later."
      );
    } finally {
      setLoading(false);
    }
  ...
  const generateDriverPlaceholder = useCallback((driverId) => {
    ...
    const colorIndex =
      driverId.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0) %
      colors.length;
  ...
  const loadPodiumPictures = useCallback(async () => {
    ...
    const picturePromises = topDrivers.map(async (driver) => {
      if (
        !loadingRef.current.has(driver.driverId) &&
        !pictures[driver.driverId]
      ) {
        ...
        try {
          ...
          pictures[driver.driverId] = generateDriverPlaceholder(
            driver.driverId
          );
        } finally {
    ...
    driversData,
    driverPictures,
    podiumPicturesLoaded,
    getDriverPicture,
    generateDriverPlaceholder,
  ]);
  ...
  const loadDriverPicture = useCallback(
    async (driver) => {
      if (
        driverPictures[driver.driverId] ||
        loadingRef.current.has(driver.driverId)
      )
        return;
  ...
  const handleItemSelect = useCallback(
    async (item) => {
      ...
      if (
        activeTab === "drivers" &&
        !driverPictures[item.driverId] &&
        !loadingRef.current.has(item.driverId)
      ) {
  ...
  const LoadingState = () => (
    <div className="min-h-screen flex flex-col items-center justify-center bg-linear-to-br from-gray-950 via-black to-gray-900 p-4">
      ...
      <div className="relative w-full max-w-md">
        {/* Outer glow effect */}
        <div className="absolute -inset-4 bg-linear-to-r from-red-900/20 via-red-600/10 to-red-900/20 rounded-full blur-xl animate-pulse"></div>

        {/* Animated rings */}
        <div className="relative flex items-center justify-center">
          {/* Outer static ring */}
          <div className="w-40 h-40 border-2 border-gray-800 rounded-full"></div>

          {/* Rotating ring - Championship trophy vibe */}
          <div className="absolute w-40 h-40 border-4 border-red-600 border-t-transparent border-r-transparent rounded-full animate-spin"></div>

          {/* Inner rotating ring (reverse direction) */}
          <div className="absolute w-28 h-28 border-3 border-red-500/50 border-b-transparent border-l-transparent rounded-full animate-spin-slow"></div>

          {/* Center emblem */}
          <div className="absolute flex items-center justify-center w-16 h-16 bg-linear-to-br from-gray-900 to-black rounded-full border border-gray-800">
            <div className="w-8 h-8 bg-linear-to-br from-red-600 to-red-700 rounded-full animate-pulse"></div>
          </div>
        </div>
        ...
        <div className="mt-12 space-y-6">
          <div className="relative">
            ...
            <div className="w-full h-2 bg-gray-900 rounded-full overflow-hidden border border-gray-800">
              {/* Animated fill */}
              <div className="h-full bg-linear-to-r from-transparent via-red-600 to-transparent animate-shimmer"></div>
            </div>
          ...
          <div className="text-center space-y-2">
            <p className="text-gray-300 font-semibold tracking-wider text-lg">
            ...
            <p className="text-gray-500 text-sm font-medium tracking-wide animate-pulse">
              Preparing tournament dashboard...
            </p>
          </div>

          {/* Animated stats loading preview */}
          <div className="grid grid-cols-3 gap-4 mt-8">
            {["TEAMS", "MATCHES", "STATS"].map((stat, index) => (
              <div
                key={stat}
                className="text-center p-3 bg-gray-900/50 rounded-lg border border-gray-800 backdrop-blur-sm"
              >
                <div className="h-3 w-12 bg-gray-800 rounded mx-auto mb-2 animate-pulse"></div>
  ...
  const ErrorState = () => (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-gray-950 via-black to-gray-900 p-6">
      <div className="max-w-md text-center bg-gray-900/50 backdrop-blur-xl rounded-3xl p-10 border border-gray-800 shadow-2xl">
        ...
        <h3 className="text-3xl font-bold text-transparent mb-4 bg-linear-to-r from-white to-red-200 bg-clip-text">
          CONNECTION ERROR
        </h3>
      ...
      <div className="lg:hidden space-y-4 mb-8">
        ...
        <div className="space-y-4">
          {topDrivers.map((driver, index) => {
            const colors = [
              {
                bg: "bg-linear-to-r from-yellow-600/20 to-amber-700/10",
                border: "border-yellow-500/30",
                text: "text-yellow-400",
              },
              {
                bg: "bg-linear-to-r from-gray-500/20 to-gray-600/10",
                border: "border-gray-400/30",
                text: "text-gray-300",
              },
              {
                bg: "bg-linear-to-r from-amber-800/20 to-amber-900/10",
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  {/* Position Medal */}
                  <div
                    className={`relative w-16 h-16 rounded-full ${colorSet.bg} border ${colorSet.border} flex items-center justify-center`}
                  >
                    <span className={`text-2xl font-bold ${colorSet.text}`}>
                      {index + 1}
                    </span>
                    {index === 0 && (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-400 truncate">
                      {simplifyTeamName(driver.team.teamName)}
                    </p>
    ...
    return (
      ...
      >
        <motion.div
          ...
          className="bg-linear-to-br from-gray-900 to-black rounded-3xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-gray-800 shadow-2xl"
        ...
        >
          ...
          <div className="p-6">
            {/* Driver Image */}
            <div className="relative w-32 h-32 mx-auto mb-6 rounded-full border-4 border-gray-800 overflow-hidden bg-linear-to-br from-gray-900 to-black">
              {loadingPictures[driver.driverId] ? (
                ...
                <img
                  src={
                    driverPictures[driver.driverId] ||
                    generateDriverPlaceholder(driver.driverId)
                  }
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-linear-to-br from-gray-800/30 to-black/30 p-4 rounded-2xl border border-gray-700/30">
            ...
            <a
              ...
              className="w-full mt-6 px-4 py-3 bg-linear-to-r from-red-600 to-red-800 text-white font-bold rounded-xl hover:from-red-700 hover:to-red-900 transition-all duration-300 flex items-center justify-center gap-3"
            >
                ...
                <div className="grid grid-cols-3 gap-8">
                  {topDrivers.map((driver, index) => (
                    ...
                    >
                      {/* Driver Card */}
                      <div
                        className={`relative rounded-3xl overflow-hidden backdrop-blur-sm border transition-all duration-500 group-hover:scale-105 ${
                          index === 0
                      ...
                      >
                        {/* Position indicator */}
                        <div
                          className={`absolute -top-4 left-1/2 transform -translate-x-1/2 w-14 h-14 rounded-2xl rotate-45 border-2 shadow-xl flex items-center justify-center ${
                            index === 0
                        ...
                        >
                          <span className="text-xl font-bold text-white -rotate-45">
                            P{index + 1}
                          </span>
                        </div>
                          ...
                          <div className="relative w-full aspect-square rounded-2xl overflow-hidden border-2 border-white/10 bg-linear-to-b from-gray-900 to-black">
                            {loadingPictures[driver.driverId] ? (
                              ...
                              <img
                                ...
                                loading="lazy"
                                onError={(e) => {
                                  e.target.src = generateDriverPlaceholder(
                                    driver.driverId
                                  );
                                }}
                              />
                        ...
                        {/* Driver Info */}
                        <div className="p-4 bg-red-600/30">
                          <div className="text-center mb-3 bg-black border-2 border-red-400/20">
                      ...
                      {index === 0 && (
                        <div className="absolute top-4">
                          <div className="flex items-center gap-2 bg-linear-to-r from-yellow-600/60 to-amber-600/40 px-3 py-1.5 rounded-full border border-yellow-500/30">
  ...
  const TimelinePodium = memo(({ topDrivers }) => {
    ...
    return (
      ...
      >
        <div className="relative p-8 font-exo2">
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* Position badge */}
                    <div
                      className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full border-2 border-gray-900 flex items-center justify-center shadow-lg ${
                        index === 0
                          ? "bg-linear-to-br from-yellow-500 to-amber-600"
                          : index === 1
                          ? "bg-linear-to-br from-gray-400 to-gray-600"
  ...
  const DriverCard = memo(({ driver, index }) => {
    ...
    return (
      <motion.div
        ...
        onClick={() => handleItemSelect(driver)}
        className={`group font-orbitron relative p-4 sm:p-6 rounded-2xl border border-gray-800/50 bg-linear-to-br from-gray-900/50 to-black/50 backdrop-blur-sm cursor-pointer transition-all duration-500 hover:scale-[1.02] hover:border-red-500/30 hover:shadow-2xl hover:shadow-red-900/20 ${
      ...
      >
        <div className="relative flex items-center gap-3 sm:gap-4">
          {/* Position */}
          <div
            className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center font-bold text-xl sm:text-2xl shadow-xl ${
              driver.position <= 3
                ? driver.position === 1
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs px-1.5 py-0.5 sm:px-2 sm:py-1 text-gray-300 rounded border border-gray-700 bg-red-400/30">
                #{driver.driver.number}
              </span>
            </div>
  ...
  const DetailPanel = () => {
    ...
    const driverPicture = isDriver
      ? driverPictures[selectedItem.driverId]
      : null;
    const pictureLoading = isDriver
      ? loadingPictures[selectedItem.driverId]
      : false;
    ...
    return (
      <motion.div
        ...
        animate={{ opacity: 1, x: 0 }}
        exit={{ opacity: 0, x: 20 }}
        className="lg:sticky lg:top-6 h-fit rounded-3xl border border-gray-800/50 bg-linear-to-br from-gray-900/80 to-black/80 backdrop-blur-xl p-6 sm:p-8 shadow-2xl font-exo2"
      >
        {/* Mobile close button */}
        {showMobileDetail && (
          <button
                ...
                {isDriver ? (
                  pictureLoading && !driverPicture ? (
                    ...
                    <img
                      src={
                        driverPicture ||
                        generateDriverPlaceholder(selectedItem.driverId)
                      }
                      alt={
                        isDriver
                          ? `${selectedItem.driver.name} ${selectedItem.driver.surname}`
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-base sm:text-lg font-medium text-white">
                      {selectedItem.driver.nationality}
                    </div>
                  </div>
                </div>
                <div className="text-right">
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-black via-gray-950 to-gray-900 font-exo2">
      ...
      <div className="max-w-8xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <motion.div
          initial={{ opacity: 0, y: -30 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-8 sm:mb-12 text-center px-2 font-orbitron"
        >
          <div className="inline-flex items-center gap-2 sm:gap-3 bg-linear-to-r from-red-600/20 via-red-500/30 to-red-600/20 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full mb-6 sm:mb-8 shadow-xl sm:shadow-2xl shadow-red-900/20 border border-red-800/30 backdrop-blur-sm">
            ...
            <span className="font-bold tracking-widest text-xs sm:text-sm">
              F1 {currentData?.season} CHAMPIONSHIP
            </span>
          </div>
          ...
          <p className="text-gray-400 mt-4 sm:mt-6 max-w-3xl mx-auto text-sm sm:text-base md:text-lg px-2">
            Real-time standings for the {currentData?.season} Formula 1 season •{" "}
            {currentData?.total} entries
          </p>
        </motion.div>

        {/* Controls */}
        <div className="mb-8 sm:mb-10 space-y-4 sm:space-y-6 px-2 font-exo2">
            ...
            <div className="flex bg-gray-900/50 backdrop-blur-sm rounded-xl sm:rounded-2xl p-0.5 sm:p-1 border border-gray-800 max-w-sm sm:max-w-md w-full">
              {["drivers", "constructors"].map((tab) => (
                <button
                  ...
                  className={`group relative flex-1 px-4 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-semibold sm:font-bold transition-all duration-500 ease-out overflow-hidden ${
                    activeTab === tab
                      ? "text-white shadow-2xl shadow-red-900/30"
                ...
                >
                  ...
                  <div
                    className={`absolute inset-0 transition-all duration-500 ease-out ${
                      activeTab === tab
                        ? "bg-linear-to-r from-red-600 via-red-700 to-red-800 opacity-100"
                        : "bg-linear-to-r from-gray-900/80 to-gray-800/80 opacity-0 group-hover:opacity-100"
                    }`}
                  ></div>
                  ...
                  <div
                    className={`absolute inset-0 transition-all duration-300 ${
                      activeTab === tab
                        ? "opacity-0"
                        : "bg-gray-900/50 border border-gray-800/50 group-hover:border-gray-700/50 group-hover:bg-gray-800/30"
                    } rounded-xl sm:rounded-2xl`}
                  ></div>

                  {/* Active state indicator line */}
                  <div
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-1 w-16 sm:w-24 bg-linear-to-r from-red-500 to-orange-500 rounded-t-full transition-all duration-500 ease-out ${
                  ...
                  <div
                    className={`absolute inset-0 transition-all duration-700 ${
                      activeTab === tab
                        ? "bg-linear-to-r from-red-500/20 via-red-600/10 to-red-500/20 animate-pulse"
                        : "opacity-0"
                    }`}
                  ></div>
                  ...
                  <div className="relative flex items-center justify-center gap-3 sm:gap-4">
                    ...
                    <span
                      className={`font-bold tracking-wider transition-all duration-300 text-sm sm:text-base ${
                        activeTab === tab
                          ? "bg-linear-to-r from-white to-gray-200 bg-clip-text text-transparent"
                    ...
                    <div
                      className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full transition-all duration-500 ${
                        activeTab === tab
                          ...
                          : "bg-gray-700 scale-0 group-hover:bg-gray-600"
                      }`}
                    ></div>
                  </div>
                  ...
                  {activeTab === tab && (
                    <div className="absolute -inset-1 bg-linear-to-r from-red-600/30 to-red-800/30 rounded-xl sm:rounded-2xl blur-sm animate-ping opacity-30"></div>
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-center">
              {activeTab === "drivers" && (
                <button
                  onClick={() =>
                    setPodiumDesign(
                      podiumDesign === "modern" ? "timeline" : "modern"
                    )
                  }
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {statsMode ? "HIDE STATS" : "SHOW STATS"}
                </span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 lg:gap-8">
            <div className="text-center sm:text-left">
              <p className="text-xs sm:text-sm text-gray-500">
                Data provided by F1 API • Season {currentData?.season}
              </p>
              <p className="text-xs text-gray-600 mt-1">
                Driver photos from Wikipedia • All data updates in real-time
              </p>
            </div>
